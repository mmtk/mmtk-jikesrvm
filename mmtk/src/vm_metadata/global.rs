    ...
    match metadata_spec {
        ...
        MetadataSpec::InHeader(metadata_spec) => {
            ...
            let res: usize = if metadata_spec.num_of_bits < 8 {
                ...
                let offset = metadata_spec.bit_offset >> LOG_BITS_IN_BYTE;
                let shift = metadata_spec.bit_offset - (offset << LOG_BITS_IN_BYTE);
                ...
                debug_assert!(
                    metadata_spec.bit_offset.trailing_zeros() as usize >= LOG_BITS_IN_BYTE as usize,
...
pub(crate) fn store_metadata(
    ...
    match metadata_spec {
        MetadataSpec::OnSide(metadata_spec) => {
            if let Some(order) = atomic_ordering {
                mmtk_meta::side_metadata::store_atomic(
                    metadata_spec,
                    object.to_address(),
                    val,
                    order,
...
pub(crate) fn compare_exchange_metadata(
    ...
    // For side metadata, use the side_metadata module from mmtk-core
    match metadata_spec {
        MetadataSpec::OnSide(metadata_spec) => mmtk_meta::side_metadata::compare_exchange_atomic(
            metadata_spec,
            object.to_address(),
            old_metadata,
            new_metadata,
        ...
        MetadataSpec::InHeader(metadata_spec) => {
            ...
            if metadata_spec.num_of_bits < 8 {
                ...
                unsafe {
                    ...
                    let expected_new_val =
                        (expected_old_val & !mask) | ((new_metadata as u8) << shift);
                    meta_addr
...
pub(crate) fn fetch_add_metadata(
    ...
    // For side metadata, use the side_metadata module from mmtk-core
    match metadata_spec {
        MetadataSpec::OnSide(metadata_spec) => mmtk_meta::side_metadata::fetch_add_atomic(
            metadata_spec,
            object.to_address(),
            val,
            order,
        ...
        MetadataSpec::InHeader(metadata_spec) => {
            ...
            if metadata_spec.num_of_bits < 8 {
                ...
                unsafe {
                    (*(object.to_address() + offset).to_ptr::<AtomicU32>())
...
pub(crate) fn fetch_sub_metadata(
    ...
    // For side metadata, use the side_metadata module from mmtk-core
    match metadata_spec {
        MetadataSpec::OnSide(metadata_spec) => mmtk_meta::side_metadata::fetch_sub_atomic(
            metadata_spec,
            object.to_address(),
            val,
            order,
