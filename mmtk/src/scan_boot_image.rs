...
pub fn scan_boot_image<W: ProcessEdgesWork<VM=JikesRVM>>(tls: OpaquePointer) {
    unsafe {
        ...
        let stride = 1usize << LOG_CHUNK_BYTES;//collector.parallel_worker_count() << LOG_CHUNK_BYTES;
        trace!("stride={}", stride);
        let start = 0usize << LOG_CHUNK_BYTES; //collector.parallel_worker_ordinal() << LOG_CHUNK_BYTES;
        trace!("start={}", start);
...
pub struct ScanBootImageRoots<E: ProcessEdgesWork<VM=JikesRVM>>(PhantomData<E>);
...
impl <E: ProcessEdgesWork<VM=JikesRVM>> GCWork<JikesRVM> for ScanBootImageRoots<E> {
    fn do_work(&mut self, worker: &mut GCWorker<JikesRVM>, mmtk: &'static MMTK<JikesRVM>) {
        scan_boot_image::<E>(OpaquePointer::UNINITIALIZED);
    }
}